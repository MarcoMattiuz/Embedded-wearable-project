...
typedef struct {
    esp_lcd_panel_t base; //generic panel interface
    esp_lcd_panel_io_handle_t io; //I2C handle
    int reset_gpio_num; 
    int x_gap; //x or y offset
    int y_gap;
    unsigned int bits_per_pixel; //1 for sh1106 
    bool reset_level;
    bool swap_axes; //if x and y are swapped
} sh1106_panel_t;
...
static esp_err_t panel_sh1106_del(esp_lcd_panel_t *panel)
{
    // containetof: gives pointer to the full struct given a pointer to one of its members.
...
static esp_err_t panel_sh1106_init(esp_lcd_panel_t *panel)
{
    ...
    // Ensures (0,0) is top-left of the display
